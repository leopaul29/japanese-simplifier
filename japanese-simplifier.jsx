...
export default function JapaneseSimplifier() {
  ...
  const finalText = hasResult ? applySubstitutions(inputText, substitutions, activeIds) : '';
  ...
  const analyze = useCallback(async () => {
    ...
    try {
      ...
      const filledPrompt = SYSTEM_PROMPT.replaceAll('{MY_LEVEL}', myLevel)
      ...
      <header style={styles.header}>
        <div style={styles.headerInner}>
          ...
          <div>
            ...
            <p style={styles.subtitle}>Selectively simplify expressions to your comfort level</p>
          </div>
        </div>
      </header>
        ...
        <section style={styles.card}>
          ...
          <div style={styles.levelRow}>
            <div style={styles.levelGroup}>
              ...
              {textType && (
                <p style={styles.typeHint}>{TEXT_TYPES.find((t) => t.id === textType)?.hint}</p>
              )}
            </div>
          </div>
          ...
          <div style={styles.levelRow}>
            <div style={styles.levelGroup}>
              <label style={styles.smallLabel}>
                My JLPT level — find expressions at this level and offer simpler alternatives
              </label>
              <div style={styles.levelPills}>
                {JLPT_LEVELS.filter((l) => JLPT_LEVELS.indexOf(l) > 0).map((l) => (
                  ...
                  >
                    {l}
                    {myLevel === l && (
                      <span style={styles.pillArrow}>
                        {' '}
                        → {JLPT_LEVELS[JLPT_LEVELS.indexOf(l) - 1]}
                      </span>
                    )}
                  </button>
                ))}
          ...
          >
            {loading ? <span style={styles.spinner}>◌ Analyzing…</span> : 'Analyze & Simplify'}
          </button>
              ...
              <div style={styles.resultHeader}>
                ...
                {substitutions.length > 0 && (
                  <button style={styles.toggleAllBtn} onClick={toggleAll}>
                    {activeIds.size === substitutions.length ? 'Deselect all' : 'Select all'}
                  </button>
                )}
              </div>
                      ...
                      {substitutions.map((sub) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td style={{ ...styles.td, ...styles.jpCell }}>{sub.original}</td>
                            <td
                              style={{
                                ...styles.td,
                                ...styles.jpCell,
