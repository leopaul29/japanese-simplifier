  ...
  <div class="relative w-full max-w-3xl overflow-hidden rounded-2xl bg-white text-slate-900 shadow-xl dark:bg-slate-950 dark:text-slate-100">
    ...
    <div class="px-6 py-8 sm:px-8">
      ...
      <div class="grid gap-5">
        <div>
          <div class="text-sm font-semibold text-slate-600 dark:text-slate-400">Provider</div>
          <div class="mt-2 flex flex-wrap gap-2" id="providerPillsContainer">
            <button class="pill pill-amber active" id="pill-openai" onclick="selectProvider('openai')">OpenAI</button>
            <button class="pill pill-amber" id="pill-gemini" onclick="selectProvider('gemini')">Gemini</button>
            <button class="pill pill-amber" id="pill-anthropic" onclick="selectProvider('anthropic')">Anthropic</button>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="mt-2 flex flex-wrap gap-4">
            <label class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
              <span id="previewPending" class="hl-pending rounded px-2 py-1">未変更</span>
              <input type="color" id="colorPending" value="#b8860b" oninput="updateHighlightColor('pending', this.value)" />
              Not replaced
            </label>
            <label class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
              <span id="previewApplied" class="hl-applied rounded px-2 py-1">変更済</span>
