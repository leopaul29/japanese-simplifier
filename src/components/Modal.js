...
<div id="settings-modal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
  <div class="relative w-full max-w-3xl overflow-hidden rounded-2xl bg-white text-slate-900 shadow-xl dark:bg-slate-950 dark:text-slate-100">
      ...
      <div class="mb-5 flex flex-col gap-2">
        <div class="flex items-center justify-between gap-4">
          <div>
            ...
            <p id="settingsStatus" class="mt-1 text-sm text-slate-500 dark:text-slate-400"></p>
          </div>
        </div>
      </div>
        ...
        <aside class="w-full md:w-56 shrink-0 pr-6 border-r border-slate-100 dark:border-slate-800">
          <div>
            ...
            <div class="mt-2 flex flex-col gap-1">
              <button class="menu-link text-left px-2 py-1 rounded hover:bg-slate-100 dark:hover:bg-slate-900" data-target="panel-api" data-provider="openai">OpenAI</button>
              <button class="menu-link text-left px-2 py-1 rounded hover:bg-slate-100 dark:hover:bg-slate-900" data-target="panel-api" data-provider="gemini">Gemini</button>
              <button class="menu-link text-left px-2 py-1 rounded hover:bg-slate-100 dark:hover:bg-slate-900" data-target="panel-api" data-provider="anthropic">Anthropic</button>
            </div>
          </div>
          ...
          <div>
            <div id="panel-api">
              <div class="grid gap-5">
                ...
                <div>
                  <div class="text-sm font-semibold text-slate-600 dark:text-slate-400">Autocomplete model <span class="ml-1 hidden rounded-full border border-blue-200 bg-blue-50 px-2 py-0.5 text-[9px] font-semibold text-blue-700 dark:border-blue-500 dark:bg-blue-950 dark:text-blue-200" id="acApiBadge">API</span></div>
                ...
                <div>
                  <div class="text-sm text-slate-600 dark:text-slate-400">Selected provider: <span id="currentProvider" class="font-semibold">OpenAI</span></div>
                </div>
              </div>
            </div>
...
;(function setupSettingsMenu(){
  ...
  const panels = settingsModal.querySelectorAll('#panel-api, #panel-highlight');
  ...
  window.selectProvider = function(provider){
    const current = settingsModal.querySelector('#currentProvider');
    if (current) current.textContent = provider.charAt(0).toUpperCase() + provider.slice(1);
    menuLinks.forEach(b => {
      if (b.dataset.provider === provider) b.classList.add('font-semibold'); else b.classList.remove('font-semibold');
    });
  };

  // initial state
  showPanel('panel-api');
  // mark first provider active
  const firstProvider = settingsModal.querySelector('.menu-link[data-provider]');
  if (firstProvider) {
    firstProvider.classList.add('bg-slate-100','dark:bg-slate-900','font-semibold');
    const p = firstProvider.dataset.provider || 'openai';
