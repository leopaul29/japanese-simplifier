...
const TEXT_TYPES = [
  {
    id: 'resume',
    label: '📄 Resume / CV',
    hint: 'Written formal — 貴社 preferred, keigo expected',
  },
  {
    id: 'email',
  ...
  },
  {
    ...
    hint: 'Spoken formal — 御社 preferred, flag written-only forms',
  },
...
const PROVIDERS = {
  ...
  gemini: {
    ...
    hint: 'Get your free key at <a href="https://aistudio.google.com/app/apikey" target="_blank">aistudio.google.com</a> — free tier available',
  },
  anthropic: {
    name: 'Anthropic',
...
async function analyze() {
  ...
  try {
    ...
    if (sel) {
      ...
      state.model =
        state.model ||
        localStorage.getItem('jlm_model_' + state.provider) ||
        PROVIDERS[state.provider].models[0];
    }
...
function updateToggleAllBtn() {
  ...
  btn.textContent =
    state.activeIds.size === state.substitutions.length ? 'Deselect all' : 'Select all';
}
