...
const FETCH_ENDPOINTS = {
  openai: async (key) => {
    ...
    return (data.data || []).map((model) => model.id).filter((id) => id.startsWith("gpt-")).sort();
  },
  gemini: async (key) => {
...
export function setupAutocomplete(inputId, suggestionsId, data, options = {}) {
  ...
  const container = options.containerId ? document.getElementById(options.containerId) : input?.parentElement;
  ...
  function groupedItems(query) {
    ...
    const filtered = q
      ? items.filter((item) => item.value.toLowerCase().includes(q))
      : items;
  ...
  function render() {
    ...
    groupEntries.forEach(([source, groupItems]) => {
      ...
      groupItems.forEach((item) => {
        ...
        name.innerHTML = query.trim() ? highlight(item.value, query.trim()) : escapeHtml(item.value);
  ...
  return {
    ...
    addData(nextData, source = "api") {
      ...
      items = [
        ...incoming,
        ...items.filter((item) => !incomingValues.has(item.value)),
      ];
      render();
    },
    getValue() {
